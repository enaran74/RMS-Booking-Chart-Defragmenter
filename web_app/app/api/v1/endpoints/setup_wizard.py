...
def _generate_secure_secret_key(prefix: str = "", username: str = "") -> str:
    ...
    # Include username for uniqueness if provided
    if username:
        # Hash the combination to create a deterministic but secure key
...
def _generate_secure_database_password() -> str:
    """Generate a cryptographically secure database password"""
    # Generate a strong random password with mixed case, numbers, and symbols
    password_bytes = secrets.token_bytes(24)
    # Encode as base64 but replace URL-unsafe characters for better compatibility
    password_b64 = secrets.token_urlsafe(24)
    # Add a special character for complexity requirements
    return f"DefragDB_{password_b64}!"
...
def generate_complete_env_file(setup_data: SetupWizardData) -> str:
    """Generate a complete .env file with cryptographically secure defaults"""
    
    # Determine USE_TRAINING_DB based on system mode
    use_training_db = "true" if setup_data.systemMode == "training" else "false"
    
    # Generate cryptographically secure keys
    secret_key = _generate_secure_secret_key("dhp-secret-", setup_data.adminUsername)
    jwt_secret_key = _generate_secure_secret_key("dhp-jwt-", setup_data.adminUsername)
    db_password = _generate_secure_database_password()
    
    # Build the complete environment configuration
    env_content = f"""# ==============================================================================
# RMS Booking Chart Defragmenter - Configuration
# ==============================================================================
# Generated by Setup Wizard on first installation with cryptographically secure keys
